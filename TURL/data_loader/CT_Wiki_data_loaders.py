...
def process_single_CT(input_data, config):
    ...
    # 存储所有的 token（即数值化的文本数据）。首先是元数据（如标题、章节标题、表格标题等），然后是表头的 token。
    input_tok = []
    # 存储每个 token 的位置索引。用于帮助模型理解 token 在序列中的位置。
    input_tok_pos = []
    # 存储每个 token 的类型。0 表示元数据，1 表示表头
    input_tok_type = []
...
class WikiCTDataset(Dataset):
    ...
    def _preprocess(self, data_dir):
        ...
        # pool.close()
        processed_cols = [process_single_CT(col, config=self) for col in tqdm(cols)]
        # pdb.set_trace()
