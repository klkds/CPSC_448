...
from util import f1_score_multilabel, f1_score_multilabel2
...
if __name__ == "__main__":
    ...
    if args.from_scratch:
        tag_name = "model/turlturl-re-colpair_mosato_bert_bert-base-uncased-bs16-ml-16__turl-1.00_turl-re-1.00=turl"
    else:   
        tag_name = "model/turlturl-re-colpair_mosato_bert_bert-base-uncased-bs16-ml-16__turl-1.00_turl-re-1.00=turl"
    # TODO: Check
    ...
    for i, num_classes in enumerate(num_classes_list):
        if args.single_col:
            ...
            if args.from_scratch:
                ...
                print("iodsfjosd")
                model.load_state_dict(torch.load("model/turlturl-re-colpair_mosato_bert_bert-base-uncased-bs16-ml-16__turl-1.00_turl-re-1.00=turl_best_micro_f1.pt", map_location=device), strict=False)
    ...
    for task in args.tasks:
        ...
        if task in [
            ...
            if task in ["turl"]:
                ...
                filepath = "data/delete_train_col_type_serialized.pkl"
                if args.single_col:
    ...
    for epoch in range(num_train_epochs):
        for k, (task, model, train_dataset, valid_dataset, train_dataloader,
            ...
            if "sato" in task:
                ...
                tr_micro_f1, _ = f1_score_multilabel2(
            ...
            if "sato" in task:
                ...
                vl_micro_f1, _ = f1_score_multilabel2(
            ...
            loss_info_list.append([
                tr_loss, tr_micro_f1, vl_loss,
                vl_micro_f1
            ])
            t2 = time()
            print(
                "Epoch {} ({}): tr_loss={:.7f} tr_micro_f1={:.4f} "
                .format(epoch, task, tr_loss, tr_micro_f1),
                "vl_loss={:.7f} vl_micro_f1={:.4f} ({:.2f} sec.)"
                .format(vl_loss, vl_micro_f1, (t2 - t1)))

    for task, loss_info_list in zip(args.tasks, loss_info_lists):
        loss_info_df = pd.DataFrame(loss_info_list,
                                    columns=[
                                        "tr_loss",
                                        "tr_f1_micro_f1", "vl_loss", "vl_f1_micro_f1"
                                    ])
        if len(args.tasks) >= 2:
